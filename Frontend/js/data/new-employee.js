// new-employee.js
// All data is read from and written to HRStore (hr-data-store.js).
...
function validateAll() {
  ...
  // Duplicate email check against live store
  ...
  if (emailEl && !emailEl.classList.contains("is-invalid")) {
    ...
    if (HRStore.getAll().some((e) => e.email.toLowerCase() === emailVal)) {
...
function populateSystemRoles() {
  // Derive unique roles from the current store so new roles auto-appear
  const sel = document.getElementById("systemRole");
  const roles = [...new Set(HRStore.getAll().map((e) => e.role))].sort();
  // Always ensure the canonical roles are present even if store is sparse
  const canonical = [
    "Project Manager",
    "Team Leader",
    "Team Member",
...
function populateReportsTo() {
  const managers = HRStore.getManagers();
...
// ═══════════════════════════════════════════════════════
// SUBMIT — writes to HRStore (updates sessionStorage)
// ═══════════════════════════════════════════════════════
function handleSubmit() {
  ...
  // Simulate a brief async save (matches UX expectation; swap with fetch() for real backend)
  setTimeout(() => {
    const result = HRStore.add(payload);

    btn.classList.remove("loading");
    btn.innerHTML = '<i class="ri-check-line"></i> Create & Provision Account';

    if (!result.ok) {
      // Map store validation errors back to form fields
      const fieldMap = {
        name: ["firstName", "lastName"],
        email: ["workEmail"],
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Populate all dropdowns from the live store — nothing hardcoded
  populateDepartments();
  populateSystemRoles();
  populateReportsTo();
