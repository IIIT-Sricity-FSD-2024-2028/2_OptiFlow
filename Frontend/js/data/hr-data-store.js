...
(function (global) {
  ...
  function _hrRoleToAuthRole(hrRole) {
    const r = String(hrRole || "").trim().toLowerCase();
  ...
  function _authRoleToHrRole(authRole) {
    const r = String(authRole || "").trim().toLowerCase();
    const map = {
  ...
  function _upsertAuthUserFromEmployee(emp) {
    if (typeof global.getUsers !== "function" || typeof global.saveUsers !== "function")
      return;
    ...
    const status =
      emp.status === "inactive" ? "Inactive" : emp.status === "pending" ? "Pending" : "Active";
    ...
    if (emp.parentId) {
      ...
      if (mgr) {
        ...
        const mgrUser =
          (idx > -1 ? users : global.getUsers()).find((u) => _normEmail(u.email) === mgrEmail) ||
          null;
  ...
  function _ensureEmployeeFromAuthUser(authUser) {
    ...
    if (authUser && authUser.reportsTo) {
      const mgrUser = typeof global.getUsers === "function"
        ? global.getUsers().find((u) => String(u.id) === String(authUser.reportsTo))
        : null;
      if (mgrUser) {
        const mgrEmp = emps.find((e) => _normEmail(e.email) === _normEmail(mgrUser.email));
    ...
    if (idx > -1) {
      ...
      existing.status = String(authUser.status || "Active").toLowerCase() === "inactive" ? "inactive" : "active";
      ...
      emps.push({ 
        ...
        status: String(authUser.status || "Active").toLowerCase() === "inactive" ? "inactive" : "active",
        joined: authUser.joined || new Date().toLocaleDateString("en-IN", { month: "short", year: "numeric" }),
        joinDateRaw: "",
  ...
  function _seedActivity() {
    return {
      "EMP-001": [
        { id: 1, text: "Created Process Admin account for OfficeSync demo", date: "Oct 01, 2024" },
        { id: 2, text: "Enabled global module access & unified audit logging", date: "Mar 28, 2026" },
      ],
      "EMP-002": [
        { id: 1, text: "Profile created and account provisioned", date: "Jan 04, 2021" },
        { id: 2, text: "Onboarded Anil Tiwari into IT Security", date: "Mar 07, 2023" },
      ],
      "EMP-003": [
        { id: 1, text: "Published ISO 27001 evidence requirements", date: "Mar 26, 2026" },
        { id: 2, text: "Reviewed evidence queue (ISO 27001)", date: "Mar 30, 2026" },
      ],
      "EMP-004": [
        { id: 1, text: "Created project: Q4 Financial Audit", date: "Mar 24, 2026" },
        { id: 2, text: "Created project: ISO 27001 Certification", date: "Mar 24, 2026" },
      ],
      "EMP-005": [
        { id: 1, text: "Assigned as Finance Team Leader", date: "Mar 16, 2022" },
        { id: 2, text: "Submitted Q4 audit evidence package", date: "Mar 29, 2026" },
      ],
      "EMP-008": [
        { id: 1, text: "Assigned as IT Security Team Leader", date: "Jun 02, 2022" },
        { id: 2, text: "Completed ISO access review checklist", date: "Mar 31, 2026" },
      ],
    };
  }
