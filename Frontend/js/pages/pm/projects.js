...
window.ProjectsPage = {
  ...
  projectCard(p) {
    ...
    const assignedUserIds = [
      ...new Set(projectTasks.map((t) => t.assignedUserId)),
    ];
    ...
    const avatars = team
      ...
      .map(
        (u, i) =>
          `<div style="width:28px;height:28px;border-radius:50%;background:var(--${u.avatarColor || "blue"});border:2px solid #fff;color:#fff;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;${i > 0 ? "margin-left:-10px" : ""}">${u.avatar}</div>`,
      )
      .join("");
    const extraAvatar =
      team.length > 3
        ? `<div style="width:28px;height:28px;border-radius:50%;background:#94a3b8;border:2px solid #fff;color:#fff;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;margin-left:-10px">+${team.length - 3}</div>`
        : "";
  ...
  bindEvents() {
    ...
    document.addEventListener("click", () => {
      document
        .querySelectorAll(".proj-dropdown")
  ...
  applyFilter() {
    ...
    this.filtered = this.state.projects.filter((p) => {
      ...
      const matchDept =
        !deptName || (dept && dept.name.toLowerCase().includes(deptName));
  ...
  openEditModal(id) {
    ...
    window.Validator.attachLive("edit-proj-name", {
      required: true,
      minLength: 3,
    });
    window.Validator.attachLive("edit-proj-desc", {
      required: true,
      minLength: 10,
    });
  },
  ...
  submitEdit(id) {
    ...
    const progress = Math.min(
      100,
      Math.max(0, parseInt(window.Helpers.getVal("edit-proj-progress")) || 0),
    );
    ...
    this.state.projects[idx] = {
      ...
      endDate:
        window.Helpers.getVal("edit-proj-due") ||
        this.state.projects[idx].endDate,
      status: statusVal,
      statusLabel: labelMap[statusVal] || statusVal,
      progress: progress,
    };
