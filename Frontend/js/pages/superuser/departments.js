...
document.addEventListener('DOMContentLoaded', () => {
    refreshDeptTable();
    document.getElementById('searchInput').addEventListener('input', refreshDeptTable);
...
function refreshDeptTable() {
    ...
    if (search) {
        depts = depts.filter(d => d.name.toLowerCase().includes(search) || d.head.toLowerCase().includes(search));
    }
...
function renderDeptTable(data) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">No departments found</td></tr>';
        return;
    }
    ...
    data.forEach(d => {
        ...
        tr.innerHTML = `
            ...
            <td>
                ...
                <button class="action-btn" style="color: #DC2626; border: 1px solid #DC2626;" onclick="deleteDept('${d.id}')">Delete</button>
            </td>
        `;
...
function saveDept() {
    ...
    if (id) {
        ...
        depts.push({
            id: 'd' + Date.now(),
            name, head, status, users: 0, processes: 0
        });
    }
