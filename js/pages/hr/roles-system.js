...
// Reads all data from HRStore (employees) + RolesStore (permissions).
// Saves to RolesStore which persists in localStorage.
...
function renderRoleList(activeKey) {
  const allEmployees = HRStore.getAll();
  const allRoles = RolesStore.getAllSystemRoles();
...
function renderEmployeesPanel(roleKey) {
  const employees = HRStore.getAll().filter((e) => e.role === roleKey);
...
function selectRole(roleKey) {
  ...
  const roleConf = RolesStore.getSystemRole(roleKey);
  originalPerms = { ...roleConf.permissions };
  currentPerms = { ...roleConf.permissions };

  // Header
  const empCount = HRStore.getAll().filter((e) => e.role === roleKey).length;
  ...
  renderEmployeesPanel(roleKey);
  renderRoleList(roleKey);
  setDirty(false);
}

// ─── Save / Discard ───────────────────────────────────────
function saveChanges() {
  RolesStore.saveSystemRole(activeRoleKey, currentPerms);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (firstRole) selectRole(firstRole.key);
