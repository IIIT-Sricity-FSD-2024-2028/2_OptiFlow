// employee-detail.js
// ═══════════════════════════════════════════════════════════════
// Reads/writes from HRStore (API-backed).
...
async function renderHero() {
  const manager = emp.parentId ? await HRStore.getById(emp.parentId) : null;
...
async function renderPage() {
  ...
  const personalDetailsHtml = await renderPersonalDetails();
  const rolePermissionsHtml = await renderRolePermissions();
  const activityHtml = await renderActivity();

  const content = document.getElementById("pageContent");
  content.innerHTML = `
    ${heroHtml}
    <div class="ed-grid">
      <div id="leftCol">${personalDetailsHtml}</div>
...
async function saveEdit() {
  ...
  const result = await HRStore.update(emp.id, payload);
...
function openStatusModal() {
  ...
  confirmBtn.onclick = async () => {
    await HRStore.setStatus(emp.id, newStatus);
    emp = await HRStore.getById(emp.id);
