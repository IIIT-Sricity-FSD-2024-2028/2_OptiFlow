...
export class ExecutiveController {
  ...
  async getMetrics(
    ...
    const teamBreakdown = teamsData.map((t) => {
      const activeTasks = t.projects.reduce(
    ...
    if (effectiveBranchId) {
      const bp = await this.prisma.project.findMany({
        where: { team: { branchId: effectiveBranchId } },
        select: { id: true },
      });
    ...
    const violationFilter =
      effectiveBranchId && branchProjectIds
        ? {
            companyId,
            status: { in: ['Open', 'Under_Review'] },
            entityType: 'Project',
            entityId: { in: branchProjectIds },
          }
        : { companyId, status: { in: ['Open', 'Under_Review'] } };

    const complianceViolationsRaw =
    ...
    if (effectiveBranchId && branchProjectIds) {
      ...
      const branchTasks = await this.prisma.task.findMany({
        where: { projectId: { in: branchProjectIds } },
        select: { id: true },
      });
    ...
    const openEscalations = await this.prisma.escalation.findMany({
      ...
      include: {
        task: {
          select: { title: true, assignedTo: { select: { fullName: true } } },
        },
    ...
    const formattedEscalations = openEscalations.map((e) => ({
      ...
      assignedOwner:
        e.task?.assignedTo?.fullName || e.reportedBy?.fullName || 'Unassigned',
